...
- Add RLS INSERT policy for anon role (Route Handler uses anon client via createClient from server.ts, which uses the anon key - NOT service role). The Route Handler needs to INSERT download records.
- Add RLS SELECT policy for anon role on contacts WHERE email_verified = true (the Route Handler needs to query contacts to check email_verified status; the existing RLS SELECT policy only allows select WHERE verification_token IS NOT NULL, which won't work after verification clears the token)
...
- Create Supabase client via `createClient()` from `@/lib/supabase/server` (this uses the anon key, same as all other server-side code in this project)
- Query contacts table: SELECT id, email_verified WHERE email = email.toLowerCase().trim() AND email_verified = true. Use .single(). If error or no data, return 403 'Unauthorized - email not verified'
- Log download: INSERT into book_downloads { contact_id: data.id, book_type }. Log errors to console but do NOT block the download if logging fails
