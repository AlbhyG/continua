...
> **Status (September 20, 2026):** carried over from the closed PR #50 (branch `fix/24-cognitive-empathy`). The "#50 / #24" section below reviewed item edits to the old empathy pool. Those were superseded by #51, which rebuilt the question pools to 90 items per pole, so read that section as history. The files it mentions (`empathy-manuscript-revisions.json` and the empathy relevance review) are not on `main`; they remain on the closed branch. The #28, #29 and #36 sections are still current. The proposed replacement wording for #28 and #29 is for the author's decision and has not been accepted.
