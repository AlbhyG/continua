...
| One-time passkey setup offer | `contacts.passkey_setup_offered_at`, keyed to the authenticated account; migration 00020 | UX preference timestamp only; no biometric data. Included in contact/account retention review. Browser/device biometrics are handled by the authenticator, not collected by this prompt. |
